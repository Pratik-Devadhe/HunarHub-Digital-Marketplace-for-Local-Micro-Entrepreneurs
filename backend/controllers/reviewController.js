...
const createReview = async (req, res) => {
  try {
    ...
    if (!entrepreneur_id || !rating) throw httpError("entrepreneur_id and rating are required", 400);
    if (Number(rating) < 1 || Number(rating) > 5) throw httpError("Rating must be between 1 and 5", 400);

    const row = await withTransaction(async (c) => {
      // Validate completed order/request
      if (product_id) {
      ...
      if (duplicate.rowCount) throw httpError("You have already reviewed this service or order", 409);
      ...
      const avg = await c.query(
        "SELECT ROUND(AVG(rating), 2) avg, COUNT(*)::int count FROM reviews WHERE entrepreneur_id = $1",
        [entrepreneur_id]
      );
      await c.query(
...
const getReviewById = async (req, res) => {
  try {
    const row = await withTransaction(async (c) => {
      ...
      if (!r.rowCount) throw httpError("Review not found", 404);
      return r.rows[0];
    });
...
const deleteReview = async (req, res) => {
  try {
    await withTransaction(async (c) => {
      ...
      await c.query(
        "UPDATE entrepreneur_profiles SET average_rating = COALESCE($1, 0), total_reviews = $2 WHERE id = $3",
        [avg.rows[0].avg || 0, avg.rows[0].count, old.rows[0].entrepreneur_id]
      );
    });
