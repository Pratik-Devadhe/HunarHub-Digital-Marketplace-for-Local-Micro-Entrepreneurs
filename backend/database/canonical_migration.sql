-- ============================================================
-- HunarHub Canonical Migration Patch
-- Idempotent, safe migration script for all platforms
-- ============================================================
...
-- Set sample verification flags for existing approved entrepreneurs
...
-- 3. Extend Service Requests Table & Foreign Keys
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_favorite_user_entrepreneur ON favorites(user_id, entrepreneur_id) WHERE entrepreneur_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS uq_favorite_user_product ON favorites(user_id, product_id) WHERE product_id IS NOT NULL;
...
-- 9. Update Orders & Order Items
ALTER TABLE orders ADD COLUMN IF NOT EXISTS entrepreneur_id BIGINT;
ALTER TABLE orders DROP CONSTRAINT IF EXISTS order_entrepreneur_fk;
ALTER TABLE orders ADD CONSTRAINT order_entrepreneur_fk 
    FOREIGN KEY (entrepreneur_id) REFERENCES entrepreneur_profiles(id) ON DELETE RESTRICT;

ALTER TABLE order_items ADD COLUMN IF NOT EXISTS status VARCHAR(30) NOT NULL DEFAULT 'PENDING';
ALTER TABLE order_items DROP CONSTRAINT IF EXISTS order_item_status_check;
ALTER TABLE order_items ADD CONSTRAINT order_item_status_check
CHECK (status IN (
    'PENDING', 'CONFIRMED', 'PROCESSING', 'READY', 'COMPLETED', 'CANCELLED'
));

-- Update order status and payment_status check constraints
